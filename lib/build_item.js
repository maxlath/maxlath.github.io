...
const buildTitleAndCover = data => {
  ...
  if (subtitle) titleHtml += `<p><strong>${subtitle}</strong></p>`
  ...
  if (showImageInContent) {
    ...
    if (imageCredits != null) {
      if (imageCreditsLink != null) {
        imageCredits = `<a href='${imageCreditsLink}' target='_blank' rel='noopener'>${imageCredits}</a>`
        imageCredits = `Picture credits: ${imageCredits}`
      } else {
        imageCredits = `Picture credits: ${marked(imageCredits).replace(/^\s*<p>/, '').replace(/<\/p>\s*$/, '')}`
      }
